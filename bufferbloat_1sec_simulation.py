import os
import numpy as np
from simulator import discrete_event_simulator, poisson_packet_generator, bursty_packet_generator
...
def run_bufferbloat_simulation(output_folder, iterations=10):
    ...
    packet_generators = [
        ...
        bursty_packet_generator(350, 30),  # Bursty traffic for the bufferbloat scenario
    ]
    ...
    for i in range(iterations):
        ...
        output_path = os.path.join(output_folder, f"bufferbloat_1sec_simulation_run_{i+1}.npy")
        np.save(output_path, results)
        print(f"Saved bufferbloat simulation run {i+1} to {output_path}")

if __name__ == "__main__":
    run_bufferbloat_simulation(output_folder="bufferbloat_1sec_simulation_results")
