...
def run_qoo_measurement_accuracy_stability_analysis(input_folder, nrp, nrpou, original_frequency=1000, iterations=10, noise_levels=[0.1], output_file="qoo_stability_analysis.png"):
    ...
    for result_file in result_files:    
        ...
        for noise_level in noise_levels:
            ...
            for _ in range(iterations):
                sampled_trace = sub_sample_trace_exponential_index_space(ground_truth, original_frequency=1000, sub_frequency=10)
                # Add noise to the sampled trace
                noise = np.random.normal(0, noise_level, sampled_trace.shape[0])
                sampled_trace += noise
                qoo_score = compute_qoo_score(sampled_trace, packet_loss=0, nrp=nrp, nrpou=nrpou)
                scores.append(qoo_score)
            stability_scores[noise_level] = scores
    ...
    fig, axes = plt.subplots(int(np.ceil(num_files / 2)), 2, figsize=(15, 3 * int(np.ceil(num_files / 2))), sharex=True)
    axes = axes.flatten()
    ...
    for i, (result_file, stability_scores) in enumerate(stability_results.items()):
        ...
        data = [stability_scores[noise_level] for noise_level in noise_levels]
        ax.boxplot(data, labels=noise_levels, patch_artist=True,
    ...
    axes[-2].set_xlabel("Standard deviation of measurement noise (seconds)")
    axes[-1].set_xlabel("Standard deviation of measurement noise (seconds)")
