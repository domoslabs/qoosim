...
def run_wifi_simulation(output_folder, iterations=1):
    """Run WiFi environment simulation and save results."""
    if not os.path.exists(output_folder):
        os.makedirs(output_folder)

    durations = [5000]  # seconds
    ...
    for duration in durations:
        for i in range(iterations):
            ...
            output_path = os.path.join(output_folder, f"wifi_simulation_run_{duration}_seconds_{i+1}.npy")
            np.save(output_path, results)
            print(f"Saved {duration}s WiFi simulation run to {output_path}")

if __name__ == "__main__":
    run_wifi_simulation(output_folder="wifi_simulation_results_longer_duration", iterations=5)
